...
const features = [
  { title: "7/24 Destek", desc: "Hangi saatte isterseniz yanınızdayız.", icon: Headset },
  { title: "Sabit Fiyat", desc: "Sürpriz ücret yok, şeffaf ücretlendirme.", icon: BadgeCheck },
  { title: "Uzman Şoförler", desc: "Deneyimli ekibimizle güvenli yolculuk.", icon: UserCheck },
  { title: "Lüks Filo", desc: "Yeni model VIP donanımlı araçlar.", icon: Sparkles },
];
        ...
        {features.map((f, i) => (
          ...
          >
            ...
            <div className="w-20 h-20 mb-6 relative z-10 flex items-center justify-center bg-white rounded-2xl group-hover:bg-gold transition-all duration-500 shadow-sm border border-cream-dark group-hover:border-gold">
              <f.icon size={36} strokeWidth={1.5} className="text-gold group-hover:text-white transition-colors duration-500" />
            </div>
