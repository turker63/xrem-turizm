...
const countryCodes = [
  ...
  { code: "+212", flag: "🇲 Marocco", label: "Morocco" },
  { code: "+353", flag: "🇮 Irlanda", label: "Ireland" },
...
export default function BookingForm() {
  ...
  return (
    <section id="booking" className="py-20 md:py-32 px-4 relative scroll-mt-20 overflow-hidden">
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="text-center mb-16 md:mb-24">
          ...
          >
            {tStrings.title1} <span className="text-[#dbc19f]">{tStrings.title2}</span>
          </motion.h3>
          <div className="h-1.5 w-20 bg-[#dbc19f] mx-auto mt-4 rounded-full"></div>
          <p className="text-[#dbc19f] text-[10px] md:text-xs font-bold tracking-[0.3em] uppercase mt-6 opacity-100">{tStrings.subtitle}</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 md:gap-16 items-start">
          ...
          >
            <div className="absolute top-0 left-0 w-full h-1.5 bg-[#dbc19f]"></div>
            ...
            <h4 className="text-luxury-dark font-black mb-8 uppercase tracking-widest text-[11px] flex items-center gap-2">
              <span className="w-8 h-8 rounded-full bg-white flex items-center justify-center border border-cream-dark shadow-sm">
                <Send size={14} className="text-[#dbc19f]" />
              </span> 
              {tStrings.formTitle}
            </h4>
            
            <form onSubmit={handleSubmit} className="space-y-4">
              <input required name="fullname" type="text" placeholder={tStrings.name} className="w-full bg-white border border-cream-dark p-4 h-[56px] rounded-xl text-xs font-bold outline-none text-luxury-dark focus:border-[#dbc19f] transition-all uppercase tracking-wider" />
              
              <div className="flex flex-col md:flex-row gap-4">
                <div className="relative w-full md:w-[130px]" ref={dropdownRef}>
                  <div 
                    onClick={() => setIsDropdownOpen(!isDropdownOpen)}
                    className="w-full h-[56px] bg-white border border-cream-dark rounded-xl flex items-center justify-between px-4 cursor-pointer hover:border-[#dbc19f] transition-all"
                  >
                    <span className="text-[13px] font-bold text-luxury-dark">{selectedCountry.flag} {selectedCountry.code}</span>
                    <ChevronDown size={14} className={`text-[#dbc19f] transition-transform ${isDropdownOpen ? 'rotate-180' : ''}`} />
                  </div>
                      ...
                      >
                        ...
                        <div className="max-h-[220px] overflow-y-auto custom-scrollbar">
                          {filteredCountries.map((c, idx) => (
                            ...
                            >
                              <span className="text-[11px] font-bold text-luxury-dark group-hover/item:text-[#dbc19f] transition-colors">{c.flag} {c.label}</span>
                ...
                <input 
                  ...
                  className="flex-1 bg-white border border-cream-dark p-4 h-[56px] rounded-xl text-xs font-bold outline-none text-luxury-dark focus:border-[#dbc19f] transition-all" 
                />
              </div>

              <input required name="email" type="email" placeholder={tStrings.email} className="w-full bg-white border border-cream-dark p-4 h-[56px] rounded-xl text-xs font-bold outline-none text-luxury-dark focus:border-[#dbc19f] transition-all placeholder:text-luxury-gray/50" />
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <input name="from" type="text" placeholder={tStrings.from} className="w-full bg-white border border-cream-dark p-4 h-[56px] rounded-xl text-xs font-bold outline-none text-luxury-dark focus:border-[#dbc19f] transition-all" />
                <input name="to" type="text" placeholder={tStrings.to} className="w-full bg-white border border-cream-dark p-4 h-[56px] rounded-xl text-xs font-bold outline-none text-luxury-dark focus:border-[#dbc19f] transition-all" />
              </div>
              
              <textarea required name="details" placeholder={tStrings.details} rows={4} className="w-full bg-white border border-cream-dark p-4 rounded-xl text-xs font-bold outline-none text-luxury-dark focus:border-[#dbc19f] transition-all resize-none" />
          ...
          >
            <div className="bg-cream p-8 md:p-10 rounded-[2.5rem] border border-cream-dark shadow-sm h-full flex flex-col justify-center">
              <h4 className="text-luxury-dark font-black mb-8 uppercase tracking-widest text-[11px] border-l-4 border-[#dbc19f] pl-4">{tStrings.opsCenter}</h4>
              <div className="space-y-6">
                <a href="tel:+905322855572" className="group flex items-center gap-5 p-3 -ml-3 rounded-2xl hover:bg-white/30 transition-colors">
                  <div className="w-14 h-14 bg-white border border-cream-dark rounded-xl flex items-center justify-center group-hover:bg-[#dbc19f] group-hover:border-[#dbc19f] transition-all duration-300 shadow-sm text-[#dbc19f] group-hover:text-white">
                    <Phone size={24} />
                  </div>
                  <div className="flex flex-col">
                    <span className="text-white text-lg font-black tracking-tight">+90 532 285 5572</span>
                    <span className="text-[#dbc19f] text-[10px] font-bold tracking-widest uppercase">{tStrings.supportLine}</span>
                  </div>
                </a>
                <a href="mailto:info@xremtransfer.com" className="group flex items-center gap-5 p-3 -ml-3 rounded-2xl hover:bg-white/30 transition-colors">
                  ...
                  <div className="flex flex-col">
                    <span className="text-white text-md font-black tracking-tight">info@xremtransfer.com</span>
                    <span className="text-[#dbc19f] text-[10px] font-bold tracking-widest uppercase">{tStrings.corpContact}</span>
                  </div>
                </a>
                <div className="group flex items-center gap-5 p-3 -ml-3 rounded-2xl">
                  <div className="w-14 h-14 bg-white border border-cream-dark rounded-xl flex items-center justify-center flex-shrink-0 shadow-sm text-[#dbc19f]">
                    <MapPin size={24} />
                  </div>
                  <div className="flex flex-col">
                    <span className="text-white text-[12px] font-black uppercase tracking-wider mb-0.5">Antalya, {lang === 'en' ? 'Turkey' : 'Türkiye'}</span>
                    <span className="text-white text-[11px] leading-relaxed font-bold opacity-80">Güzeloba Mah. 2262 Sok No 25/102</span>
                  </div>
                </div>
              </div>
            </div>
            <div className="h-[250px] w-full rounded-[2.5rem] overflow-hidden border-4 border-cream shadow-xl relative group">
              <iframe 
                src="https://www.google.com/maps/embed?pb=!1m14!1m8!1m3!1d102148.81104070792!2d30.805567!3d36.862820000000006!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x14c39b0db9adc119%3A0xc4ea44e07080191f!2zWFJFTSBUVVLEsFpNIExURCDFnlTEsA!5e0!3m2!1str!2str!4v1777928810580!5m2!1str!2str" 
