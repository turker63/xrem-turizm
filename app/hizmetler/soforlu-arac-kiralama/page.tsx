...
export default function SoforluAracKiralamaPage() {
  ...
  const tStrings = {
    badge: lang === 'en' ? "VIP Solutions" : "VIP Çözümler",
    title1: lang === 'en' ? "ANTALYA CHAUFFEURED" : "ANTALYA ŞOFÖRLÜ",
    title2: lang === 'en' ? "CAR RENTAL" : "ARAÇ KİRALAMA",
    heading: lang === 'en' ? "Service Details" : "Hizmet Detayları",
    para1: lang === 'en'
      ? "Are you looking for a comfortable and enjoyable tour or meeting transportation? With our specially planned VIP vehicles, we make it as easy as possible for you to get from one location to another, providing a comfortable environment for crowded groups of friends or a pleasant Antalya city tour with your nuclear family. With in-car folding tables and television connection, we provide 24/7 service with our Antalya chauffeured car rental service."
      : "Konforlu ve keyifli bir tur, toplantı ulaşımı mı arıyorsunuz? Sizler için özel olarak planlanmış VIP araçlarımız ile ister kalabalık arkadaş gruplarınız için veya çekirdek aileniz ile yapacağınız keyifli Antalya şehir turu veya toplantı salonlarına yetişebilmeniz için sadece size özel VIP araçlarımız ile istediğiniz noktadan bir diğer konuma gitmenizi olabildiğince kolaylaştırıyor ve konforlu bir ortam sunuyoruz.",
    para2: lang === 'en'
      ? "We provide high-end services for special days, including wedding organizations, bridal cars, and transporting guests to event areas. Whether it's an entertainment event, wedding, or concert, we handle the logistics of musicians and their equipment. With different vehicle options having high or low passenger capacities, we provide healthy, safe, and economic transportation services tailored to different customer demands."
      : "Özel günler için sunmuş olduğumuz Antalya şoförlü araç kiralama, Antalya transfer, Antalya Havalimanı ulaşım, servis hizmetimiz mevcuttur. Mercedes araç filomuz ile Antalya ve çevresini iyi bilen, tam donanımlı personellerimiz bulunmaktadır. Düğün organizasyonları kapsamında, Antalya gelin arabası ve özel misafirlerinizin ya da davetlilerinizin düğün alanına ulaştırılması hizmetimiz hem sizler için kolay hem de davetliler için konforlu bir tercih olacaktır.",
    para3: lang === 'en'
      ? "Our expert and experienced staff work to best meet your needs for airport or hotel transfers. Our contact number is active 24/7 to provide professional solutions for your Antalya Chauffeured Car Rental needs. You can also send your requests via email. For reservation cancellations or changes, you can contact us quickly. As Antalya airport transfer specialists, we serve Antalya and its surrounding provinces."
    ...
    details: lang === 'en' ? "Route and Special Requests..." : "Güzergah ve Özel Talepleriniz...",
  ...
  const highlights = [
    { title: lang === 'en' ? "Luxury Fleet" : "Lüks Filo", desc: lang === 'en' ? "Mercedes-Benz vehicles" : "Mercedes-Benz araçlar", icon: Star },
    { title: lang === 'en' ? "24/7 Service" : "7/24 Hizmet", desc: lang === 'en' ? "Non-stop active logistics" : "Kesintisiz aktif lojistik", icon: Clock },
    { title: lang === 'en' ? "Expert Drivers" : "Uzman Şoförler", desc: lang === 'en' ? "Experienced & certified" : "Deneyimli ve belgeli", icon: CheckCircle2 },
    { title: lang === 'en' ? "Flexible Routes" : "Esnek Rotalar", desc: lang === 'en' ? "Tailored planning" : "Size özel planlama", icon: Map },
    { title: lang === 'en' ? "Fully Equipped" : "Tam Donanım", desc: lang === 'en' ? "Desk, TV, Internet" : "Masa, TV, internet", icon: Briefcase },
  ];
            ...
            <motion.aside initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} className="lg:col-span-5 xl:col-span-4 space-y-8 lg:sticky lg:top-32">
              <div className="bg-white/80 backdrop-blur-xl border border-white p-8 md:p-10 rounded-[3rem] shadow-2xl relative overflow-hidden">
                ...
                <form onSubmit={handleSubmit} className="space-y-4 relative z-10">
                  ...
                  <button type="submit" disabled={isSubmitting} className="bg-[#1a1a1a] hover:bg-gold text-white px-8 py-5 rounded-2xl font-black uppercase text-[11px] tracking-[0.2em] transition-all shadow-xl active:scale-95 flex items-center justify-center gap-3 w-full">
